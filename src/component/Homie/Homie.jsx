...
export default function LandingPage(props) {
  ...
  const adsterraReferralLink = "https://beta.publishers.adsterra.com/referral/fbpH9hBDcx";

  const handleClaimClick = async () => {
    const trimmed = username.trim().toLowerCase();
    const isValid = /^[a-zA-Z0-9_-]{3,32}$/.test(trimmed);
    if (!isValid) {
      return toast.error("Username must be 3–32 characters and contain only letters, numbers, _ or -");
    }
  ...
  useEffect(() => {
    const checkAdUnit = async () => {
      if (session?.user?.username) {
        try {
          const res = await fetch(`/api/getPublisher?username=${session.user.username}`);
          const data = await res.json();
          const ad = data?.adUnit || {};
          const isEmpty = !ad.scriptUrl && !ad.containerId && !ad.clickUrl && !ad.apiKey;
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white flex flex-col items-center px-4 py-10">
      ...
      <div className="text-center max-w-2xl">
        <h1 className="text-4xl font-bold text-[#00f2fe]">Monetize Your Bio — Instantly</h1>
        <p className="text-gray-300 mt-4">
          With <span className="text-white font-semibold">Bio Link</span>, you get a powerful bio page, built-in monetization, and full traffic stats — <span className="text-[#00f2fe]">free</span>.
        </p>
      </div>
        ...
        showAdForm ? (
          <div className="mt-10 w-full max-w-xl space-y-5">
            <div className="bg-[#1e293b] p-5 rounded-xl border border-[#00f2fe] shadow-md text-center">
              <h2 className="text-lg font-semibold text-white mb-2">Ads powered by Adsterra</h2>
            ...
            {[ 
              { key: "id", label: "Domain ID (7-digit)", placeholder: "1234567" },
              { key: "nativeBar", label: "Native Bar Script URL", placeholder: "Paste full script src URL" },
              { key: "clickUrl", label: "Click URL", placeholder: "Enter click URL" },
              ...
              <div key={key}>
                ...
                <input
                  type="text"
                  value={adInput[key]}
                  onChange={(e) => setAdInput({ ...adInput, [key]: e.target.value })}
      ...
      <div className="mt-16 grid gap-6 grid-cols-1 sm:grid-cols-2 w-full max-w-4xl">
        <Feature title="We Provide the Ads" description="You don’t need sponsors or brands. We handle ads — you just get paid." />
        <Feature title="100% Free Forever" description="No subscription, no limits. Everything you need to grow and earn — for free." />
        <Feature title="All Your Links in One" description="Instagram, YouTube, your shop — one stylish, customizable link page." />
        <Feature title="Your Stats Dashboard" description="Track clicks, traffic, and earnings in real-time with your personal dashboard." />
      </div>
